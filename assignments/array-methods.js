...
let runnersLargeSizeShirt = runners.filter(function(person){
...
let ticketPriceTotal = runners.reduce(function(acuumalator, amount){
  return acuumalator + amount.donation;
}, 0);
console.log(`Tell Uncle Sam we raised a total of ${ticketPriceTotal} dollars but we're a nonprofit so he can't have it!`);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
let eliteFundraisers = runners.filter(function(raised){
  return raised.donation > 200;
});
console.log(eliteFundraisers);

// Problem 2
let groupA = runners.map(nameBegins => nameBegins.last_name.startsWith("A"));
console.log(groupA);

// Problem 3
let myHood = []
runners.forEach(function(represents) {
  myHood.push(`I'm ${represents.first_name} from ${represents.company_name}`);
});
console.log(myHood);